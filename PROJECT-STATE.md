...
Release line: 1.0.0 (build 1)
Verified gate: Milestone 5, 17/17 human checks
Platform: macOS 15+, Apple silicon, Swift 6, SwiftUI

## Product state

Milestones 1–5 are complete. Version 1.0 delivers the complete local workflow:
...
and optionally delete the audio while retaining the note.
...
Post-v1 work begins with `PRD-6.md`. Do not start it merely because this handoff
exists; follow the milestone gate in `CLAUDE.md`/`AGENTS.md`.
...
- `RecorderService`: microphone selection, AVAudioEngine tap, AAC/ALAC encoding,
  crash-tolerant CAF partial, final M4A remux, live waveform, pause/resume.
...
- `EntryDetailView`, `TranscriptWorkbenchView`, `WaveformView`,
  `TrimSelectionView`: the main note/audio workbench.
...
- `PRD-6.md`: command registry/palette, quick switcher, history, tabs/split,
  multi-select, bulk export, status bar. Begin by consolidating `AppCommands`,
  `KeyboardShortcutsView`, and `AppModel.installKeyMonitor` behind the registry.
- `PRD-7.md`: live Markdown presentation, wikilinks/backlinks, tags, outline,
  layer diff, templates. Extend `TranscriptWorkbenchView`, `VaultSearchIndex`, and
  frontmatter accessors without breaking unknown-field round trips.
- `PRD-8.md`: menu-bar/global capture, punch-in replace recording, non-destructive
  enhancement, metering, notifications, recovery hardening. Reuse `RecorderService`,
  the trim/trash safe-swap pattern, and the one persistent transcription seam.
- `PRD-9.md`: motion/appearance, insights, onboarding, accessibility, signed and
  notarized distribution, and opt-in update checks.
...
4. Test recording permission, model download, vault bookmarks, relaunch, and the v1
   acceptance flow from a clean macOS user account.
5. Tag the verified source as `milestone-5` and `v1.0.0`; publish release notes from
   `CHANGELOG.md` and attach only notarized artifacts.

The current machine has no valid code-signing identities, so source/tag publication
is ready but public binary distribution is not yet honest or complete.
